 ...
 * @author MANUEL
 */
public class multiplesitesreport extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String getdistinctsites="select distinct (site) as site, year,period ,cbo from backgroundinfor where ass_date between '"+startdate+"' and '"+enddate+"' order by site";
...
//            year=request.getParameter("year");
 ...
 //begin a loop that will create as many reports as possible
    for(int u=0;u<siteids.size();u++){        
            ...
            conn.rs=conn.st.executeQuery("select site_name from sites where site_id='"+siteids.get(u)+"'");
            if(conn.rs.next()){
...
//            innerdata_style.setBorderLeft(HSSFCellStyle.BORDER_THIN);
       ...
       //``````````````````````````````INNER SECTION HEADERS``````````````````            
          ...
          shet2.addMergedRegion(new CellRangeAddress(rwcount,rwcount,4,5));   
            ...
            response.setHeader("Content-Disposition", "attachment; filename=OVC_LIP_REPORT_Multiple_SITES_"+startdate+"_"+enddate+".xls");
